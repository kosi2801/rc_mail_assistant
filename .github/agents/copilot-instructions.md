...
- Python 3.11 + FastAPI, SQLAlchemy (async), Jinja2, Fernet (`cryptography`), HTMX (frontend) (005-dashboard-gmail-status)
- PostgreSQL (local Docker); read-only access from health fragment route (005-dashboard-gmail-status)
...
## Recent Changes
- 005-dashboard-gmail-status: Added Python 3.11 + FastAPI, SQLAlchemy (async), Jinja2, Fernet (`cryptography`), HTMX (frontend)
